...
class Mesh : public Figure {
   ...
   public:
    Mesh(std::vector<Vertex> const& vertices, std::vector<GLuint> const& indexes, ptr<FigureDrawer> drawer, ptr<FigureUpdater> updater)
    ...
    std::vector<GLuint> const& getIndexes() const {
        return indexes;
    }
